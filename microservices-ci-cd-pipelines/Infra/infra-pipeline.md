...
The pipeline is triggered with a parameter for targetgroup, which specifies the name of the target group dynamically.
...
The resource type AWS::ElasticLoadBalancingV2::TargetGroup is used to define an AWS Elastic Load Balancer target group.

The target group listens on port 8702 and uses the HTTP protocol.

Health checks are performed at the endpoint /dev/health with a health check interval of 10 seconds and a timeout of 9 seconds.

The VPC ID and Load Balancer ARN must be correctly specified for the environment where this stack is deployed.

Outputs:

The Output section exports the TargetGroup resource so that it can be used by other stacks or services in the AWS environment. The export name is tied to the value of the Targetgroup parameter.
...
Ensure that the target group is created successfully and is associated with the correct load balancer and VPC.
